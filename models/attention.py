...
from diffusers.models.normalization import AdaLayerNormSingle, AdaLayerNorm, AdaLayerNormContinuous, AdaLayerNormZero
from diffusers.utils import USE_PEFT_BACKEND, deprecate, is_torch_version
from einops import rearrange, repeat

from typing import Optional, Dict, Any

class FlattenTransformer2DModel(Transformer2DModel):
    def __init__(self, transformer: Transformer2DModel):
        self.__dict__.update(transformer.__dict__)
        self.load_state_dict(transformer.state_dict())
    ...
    def forward(
        ...
        hidden_states = hidden_states[0] if isinstance(hidden_states, tuple) else hidden_states.sample
        hidden_states = rearrange(hidden_states, "(b f) c h w -> b c f h w", f=video_length)
        output = Transformer2DModelOutput(sample=hidden_states) if return_dict else (hidden_states,)
        return output
    
    
class BasicCrossFrameTransformerBlock(nn.Module):
    def __init__(
        self,
        dim: int,
    ...
    def forward(
        ...
        hidden_states = attn_output + hidden_states
        hidden_states = rearrange(hidden_states, "(b p) f c -> (b f) p c", p=num_latent_pixels)
...
class CrossFrameTransformer2DModel(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self, 
        num_attention_heads: int = 8,
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicCrossFrameTransformerBlock(
                    dim=inner_dim,
